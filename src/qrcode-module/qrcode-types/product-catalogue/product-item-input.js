...
import { QrcgProductCatalogueItemModal } from './product-item-modal'
...
export class QrcgProductItemInput extends LitElement {
    static styles = [
        css`
            :host {
                display: block;
    ...
    canAddNewItem() {
        ...
        const number = currentPlan().number_of_product_catalogue_items
    ...
    openItemModal(item) {
        return QrcgProductCatalogueItemModal.open({
            data: item,
    ...
    renderEmptyItemsMessage() {
        ...
        return html`
            <div class="empty-message">
                ${t`There are no products. Click on Add Product button below.`}
            </div>
        `
    }
