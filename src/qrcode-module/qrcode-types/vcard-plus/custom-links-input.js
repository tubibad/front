...
import { QrcgCustomLinksModal } from './custom-links-modal'

export class QrcgCustomLinksInput extends LitElement {
    static styles = [
        css`
            :host {
                display: block;
    ...
    openMenuItemModal(data) {
        return QrcgCustomLinksModal.open({
            data,
        })
    }
    ...
    renderEmptyCategoriesMessage() {
        ...
        return html`
            <div class="empty-message">
                ${t`There are no custom links. Click on Add Custom Link button below.`}
            </div>
        `
    }
...
window.defineCustomElement('qrcg-custom-links-input', QrcgCustomLinksInput)
